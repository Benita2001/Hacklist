...
export async function GET() {
  ...
  const { data, error } = await client.from('opportunity_follows').select('opportunity_id, created_at').eq('user_id', user.id).order('created_at', { ascending: false });
  if (error) return NextResponse.json({ ok: false, error: 'follows_unavailable' }, { status: 503 });
...
export async function POST(request: NextRequest) {
  ...
  const { error } = await client.from('opportunity_follows').insert({ user_id: user.id, opportunity_id: input.opportunity_id });
  if (error && error.code !== '23505') return NextResponse.json({ ok: false, error: 'follow_unavailable' }, { status: 503 });
  return NextResponse.json({ ok: true, following: true, opportunity_id: input.opportunity_id });
}
...
export async function DELETE(request: NextRequest) {
  ...
  const { error } = await client.from('opportunity_follows').delete().eq('user_id', user.id).eq('opportunity_id', opportunityId);
  if (error) return NextResponse.json({ ok: false, error: 'follow_unavailable' }, { status: 503 });
  return NextResponse.json({ ok: true, following: false, opportunity_id: opportunityId });
}
