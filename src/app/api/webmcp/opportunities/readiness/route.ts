...
import { buildOpportunityReadiness, loadOpportunityByRefs, parseJsonBody, parseOpportunityRef } from '@/lib/webmcp/opportunities';
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({
    ok: true,
    tool: 'get_opportunity_readiness',
    generatedAt: new Date().toISOString(),
    readiness: buildOpportunityReadiness(opportunities[0]),
  });
}
